...
class Generator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(nz, ngf * 4, 2, (1, 2), 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm2d(ngf * 4),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose2d(ngf * 4, ngf * 2, 2, (1, 2), 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm2d(ngf * 2),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose2d(ngf * 2, ngf, 2, (1, 2), 0, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm2d(ngf),
            # state size. (ngf*4) x 8 x 8
            nn.utils.weight_norm(nn.ConvTranspose2d(ngf, nc, 2, (1, 1), 0, bias=False)),
            nn.LeakyReLU(0.2, inplace=True),
