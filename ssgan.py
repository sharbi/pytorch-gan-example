...
class Generator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(nz, ngf * 4, 3, (1, 4), 0, bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose2d(ngf * 4, ngf * 2, 3, (1, 4), 2, bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ngf*4) x 8 x 8
            nn.ConvTranspose2d( ngf * 2, ngf, 3, (1, 2), 1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ngf) x 16 x 16
            nn.ConvTranspose2d( ngf, nc, 3, (1, 2), 1, bias=False)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(diabetes_loader_train):
        ...
        unsupervised_loss = torch.mean(real_data_loss) + torch.mean(fake_data_loss)
