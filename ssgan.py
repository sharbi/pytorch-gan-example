...
class Discriminator(nn.Module):
    ...
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.utils.weight_norm(nn.Conv2d(ndf, ndf, 2, padding=1, stride=2, bias=False)),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.5),
            # (ndf) x 30 x 2
            nn.utils.weight_norm(nn.Conv2d(ndf, ndf *2, 3, padding=1, bias=False)),
            nn.LeakyReLU(0.2),
            nn.utils.weight_norm(nn.Conv2d(ndf*2, ndf*2, 2, padding=1, bias=False)),
            nn.LeakyReLU(0.2),
            nn.utils.weight_norm(nn.Conv2d(ndf*2, ndf*2, 2, padding=1, stride=2, bias=False)),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.5),
            # (ndf) x 15 x 1
